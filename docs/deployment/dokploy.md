...
| Symptom | Cause | Fix |
|---|---|---|
| Any `419` or sign-in problem | Not known yet | Start with `php artisan axispay:doctor` in the container terminal (read-only, prints no secret). It checks the `APP_URL` scheme against `SESSION_SECURE_COOKIE`, `SESSION_DOMAIN`, the hosts and their session cookie names, the session table, the cache, `TRUSTED_PROXIES` and pending migrations, and exits non-zero on an error |
...
| `419` on the **second** action of a panel page (second sign-in attempt, 2FA set-up), the first one works | A bug in versions before the fix of 2026-09-25: the panel middleware ran twice on Livewire requests and switched the session | Deploy a version that includes the fix (see `CHANGELOG.md`) |
| `419` at random, or after being signed out for no reason, with several containers or Applications | Containers that serve the same host have a different `APP_KEY`, or different session storage (`SESSION_DRIVER=file`, another database): a request that lands on the other container cannot read the session | Run `axispay:doctor` in each container: the `APP_KEY fingerprint` (first 12 hex characters of its SHA-256, not reversible) must be identical, and `SESSION_DRIVER` must be `database` on the same database. Make sure no second Application (for example an old `all-in-one`) has a domain on the same host |
| The panel reloads by itself after a long pause | The session expired (2 hours of inactivity, plan 17.3) and the panel reloaded instead of showing "This page has expired" (ADR-0040) | Expected. If it happens within minutes, run `axispay:doctor` |
