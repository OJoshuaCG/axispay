...
- **Passwords:** `Password::defaults()` = 12 characters minimum + `uncompromised()` (HIBP k-anonymity API), switchable with `PAYLINK_PASSWORD_CHECK_UNCOMPROMISED` (off in `phpunit.xml`; tested with `Http::fake()`).
